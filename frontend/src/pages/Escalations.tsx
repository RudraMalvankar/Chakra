  ...
  return <div className="max-w-7xl mx-auto space-y-6">
    ...
    {error ? <div className="bg-white border border-red-200 text-rzp-red p-6">Unable to load escalations: {error}</div> : <>
      ...
      <div className="bg-white border border-border overflow-auto">
        ...
        <tbody>{items.length ? items.map(item => { const action = nextAction(item.status); return <tr key={item.id} className="border-t border-border"><td className="p-4 font-mono text-rzp-blue"><a href={`/escalations/${item.id}`} className="hover:underline">{item.case_id}</a></td><td className="p-4">{item.reason}</td><td className="p-4"><Badge status={item.priority}>{item.priority}</Badge></td><td className="p-4">{item.assigned_to || 'Unassigned'}</td><td className="p-4"><Badge status={item.status}>{item.status}</Badge></td><td className="p-4">{action && <button disabled={updating === item.id} onClick={() => advance(item, action[0])} className="text-xs font-bold text-rzp-blue disabled:opacity-50">{updating === item.id ? 'Updating…' : action[1]}</button>}</td></tr>; }) : <tr><td colSpan={6} className="p-8 text-center text-text-muted">No escalations.</td></tr>}</tbody></table>
      </div>
    </>}
  </div>;
};
