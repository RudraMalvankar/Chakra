...
def verify_invariants(metrics: Dict[str, Any]) -> Dict[str, bool]:
    ...
    rev_attempted = metrics.get("revenue_recovery_attempted_inr", 0.0)
...
def generate_metrics_report(audit_file: Optional[str] = None) -> Dict[str, Any]:
    ...
    if not os.path.exists(log_path):
        return {
            "metrics": {
                ...
                "revenue_recovery_attempted_inr": 0.0,
                "revenue_attempted_inr": 0.0,
                "revenue_recovered_inr": 0.0,
                "revenue_pending_inr": 0.0,
                "revenue_blocked_inr": 0.0,
                "revenue_escalated_inr": 0.0,
            },
            "invariants": {
    ...
    with open(log_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if etype == "triage_decision_proposed":
                ...
                payment_pending[pid] = True
                
            elif etype == "execution_outcome":
                payment_attempted[pid] = True
                payment_pending[pid] = False
    ...
    revenue_recovery_attempted_inr = round(sum(payment_amounts[pid] for pid in payment_amounts if payment_attempted.get(pid, False)), 2)
    revenue_attempted_inr = revenue_recovery_attempted_inr # Backwards compat
    
    revenue_pending_inr = round(sum(payment_amounts[pid] for pid in payment_amounts if payment_pending.get(pid, False)), 2)
    ...
    with open(log_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if etype == "execution_blocked":
                intervention_metrics[action]["blocked"] += 1
            elif etype == "execution_escalated":
                intervention_metrics[action]["escalated"] += 1
            elif etype in ["retry_scheduled", "voice_artifact_generated", "reminder_artifact_generated"]:
    ...
    metrics = {
        ...
        "revenue_recovery_attempted_inr": revenue_recovery_attempted_inr,
        "revenue_attempted_inr": revenue_attempted_inr,
        "revenue_recovered_inr": revenue_recovered_inr,
        "revenue_pending_inr": revenue_pending_inr,
        "revenue_blocked_inr": revenue_blocked_inr,
        "revenue_escalated_inr": revenue_escalated_inr,
